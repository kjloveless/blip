...
pub const Terminal = struct {
    ...
    pub fn editorReadKey(self: Terminal) !u8 {
        var bytes_read: usize = undefined;
        var char: [1]u8 = undefined;
        while (bytes_read != 1) {
            bytes_read = self.reader.read(&char) catch |err| switch (err) {
        ...
        if (char[0] == '\x1b') {
            ...
            if (try self.reader.read(seq[0..1]) != 1) return '\x1b';
            if (try self.reader.read(seq[1..2]) != 1) return '\x1b';

            if (seq[0] == '[') {
                if (seq[1] >= '0' and seq[1] <= '9') {
                    if (try self.reader.read(seq[2..3]) != 1) return '\x1b';
                    if (seq[2] == '~') {
                        switch (seq[1]) {
    ...
    fn getCursorPosition(self: Terminal, rows: *u16, cols: *u16) !i8 {
        var buffer: [32]u8 = undefined;
        var i: usize = 0;

        if (try self.writer.write("\x1b[6n") != 4) {
            return -1;
        }

        var char: [1]u8 = undefined;
        while (i < buffer.len - 1) : (i += 1) {
            if (try self.reader.read(&char) != 1) {
                break;
            }
            buffer[i] = char[0];
            if (buffer[i] == 'R') {
    ...
    pub fn getWindowSize(self: Terminal, rows: *u16, cols: *u16) !i8 {
        ...
        if ((ioctl_result == -1) or (ws.ws_col == 0)) {
            if (try self.writer.write("\x1b[999C\x1b[999B") != 12) {
                return -1;
            }
            return getCursorPosition(self, rows, cols);
        } else {
            cols.* = ws.ws_col;
            rows.* = ws.ws_row;
            return 0;
